...
async function createUser(user, isFbLogin) {
  const obj = {
    ...
    email: user.email,
    password: isFbLogin
      ? passwordHash.generate("test123#489")
...
async function retriveFbUserDetials(userInfo) {
  const res = await FB.api("me", {
    fields: ["id, name, about, email, accounts, link"],
...
async function resetUserPassword(userDetail) {
  const isAppliedChangeRequest = await models.changePassword.findOne({
    where: { email: userDetail.email, token: userDetail.token },
  });
  if (isAppliedChangeRequest) {
    const doc = await models.Users.update(
      { password: passwordHash.generate(userDetail.password) },
      { where: { email: userDetail.email } }
    );
    return doc;
  } else throw "No change password request found.";
}
...
async function changeUserPassword(userDetail) {
  if (await checkIfUserAuth(userDetail.email, userDetail.currentPassword)) {
    ...
    return doc;
  } else throw "Current password is incorrect.";
}
